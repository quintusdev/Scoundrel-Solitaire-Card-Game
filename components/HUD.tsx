  ...
  return (
    <div className={`bg-slate-800/50 p-6 rounded-2xl border border-slate-700 flex flex-wrap gap-8 justify-between items-center mb-12 transition-all duration-300 ${effectClass && !effectClass.includes('animate-weapon-pop') ? effectClass : ''}`}>
      ...
      <div className="flex gap-8">
        ...
        <div className="text-center">
          <span className="block text-xs uppercase text-slate-500 font-bold mb-1">Stanza</span>
          <span className="text-xl font-bold">{state.roomIndex}</span>
        </div>
        <div className="text-center">
          <span className="block text-xs uppercase text-slate-500 font-bold mb-1">Mazzo</span>
          <span className="text-xl font-bold text-slate-400">{state.deck.length}</span>
        </div>
      </div>

      <div className="bg-slate-900/50 px-4 py-2 rounded-lg border border-slate-700 min-w-[150px]">
        {selectedCard ? (
          <div>

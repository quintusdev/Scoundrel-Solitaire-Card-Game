...
const HUD: React.FC<{ state: GameState, eternalVariant?: EternalVariant | null, worldShifts?: WorldShift[] }> = ({ state, eternalVariant, worldShifts = [] }) => {
  ...
  return (
    <div className={`hud-glass p-6 rounded-[32px] flex flex-col gap-6 mb-6 border-2 transition-all duration-500 ${isQuestion ? 'border-cyan-500/40' : isGod ? 'border-yellow-500/30' : 'border-white/10'}`}>
      ...
      {isQuestion && worldShifts.length > 0 && (
        <div className="border-t border-cyan-500/20 pt-4">
          <span className="text-[8px] uppercase font-black text-cyan-600 tracking-widest mb-2 block">Anomalie Attive</span>
          <div className="flex flex-wrap gap-2">
            {worldShifts.map(s => (
              <div key={s.id} className={`px-2 py-1 rounded-md text-[8px] font-black uppercase tracking-tighter border ${s.category === 'hostile' ? 'bg-red-950/20 text-red-500 border-red-900/40' : s.category === 'beneficial' ? 'bg-emerald-950/20 text-emerald-500 border-emerald-900/40' : 'bg-cyan-950/20 text-cyan-500 border-cyan-900/40'}`}>
                {s.name}
              </div>
            ))}
          </div>
